...
//        Wait_One_Sec_Soft();
        ...
        {
            tempSecond = second;
            char tempC = DS1621_Read_Temp(); //gain temperature input
            char tempF = (tempC * 9 / 5) + 32; //convert temperature to F
        ...
        if (Nec_OK == 1)
        {
            ...
            // Pulling to look for code using a FOR loop
            for (char i=0; i<21; i++)
            {
...
//                        printf ("array1[i] = %x \r\n", array1[i]); 
			...
			// Code for KEY_PRESSED LED accompanying buzzer sound
                KEY_PRESSED = 1;
                Activate_Buzzer();
                Wait_One_Sec();
                Deactivate_Buzzer();
